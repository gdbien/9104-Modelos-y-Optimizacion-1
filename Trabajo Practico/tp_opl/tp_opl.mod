...
int DPP = DESTINOS_POR_PASADA; //Tiene que tener el mismo nombre para que lo cargue, y ni loco voy a usar
							   //DESTINOS_POR_PASADA que es un CHOCLO de variable
...
//Como no se extraer de la formula los nodos de la ultima pasada, uso una cuenta auxiliar
float aux5 = (((DPP - 1) * cantTotNodos) + 1)/ DPP;
int nodosUltNivel = ftoi(aux5);

int indIniUltNivelPas = cantTotNodos-nodosUltNivel;
...
subject to {
	...
	//Restriccion de simetria que falta plantear bien
